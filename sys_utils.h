...
#ifndef CAL_SYS_UTILS_HPP
#define CAL_SYS_UTILS_HPP
...
template <typename T>
using AlignedVector = std::vector <T, cal::AlignedAllocator <T> >;

// Define these explicitly, since specialization of using directive is not always
// possible without wrapping in a struct.

using AlignedI8 = std::vector <int8_t, cal::AlignedAllocator <int8_t> >;
using AlignedU8 = std::vector <uint8_t, cal::AlignedAllocator <uint8_t> >;
using AlignedI16 = std::vector <int16_t, cal::AlignedAllocator <int16_t> >;
using AlignedU16 = std::vector <uint16_t, cal::AlignedAllocator <uint16_t> >;
using AlignedI32 = std::vector <int32_t, cal::AlignedAllocator <int32_t> >;
using AlignedU32 = std::vector <uint32_t, cal::AlignedAllocator <uint32_t> >;
using AlignedI64 = std::vector <int64_t, cal::AlignedAllocator <int64_t> >;
using AlignedU64 = std::vector <uint64_t, cal::AlignedAllocator <uint64_t> >;
using AlignedF32 = std::vector <float, cal::AlignedAllocator <float> >;
using AlignedF64 = std::vector <double, cal::AlignedAllocator <double> >;
}

#endif // ifndef CAL_UTILS_HPP
