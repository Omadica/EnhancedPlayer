...
void cal::qa_list_dot(size_t n, size_t m, size_t d, double const * a,
                        double const * b, double * dotprod) {
    if (cal::is_aligned(a) && cal::is_aligned(b) &&
        cal::is_aligned(dotprod)) {
        #pragma omp simd
...
void cal::qa_inv(size_t n, double * q) {
    if (cal::is_aligned(q)) {
        #pragma omp simd
        for (size_t i = 0; i < n; ++i) {
...
void cal::qa_amplitude_one(size_t d, double const * v,
                             double * norm) {
    double dotprod = 0.0;
...
void cal::qa_amplitude(size_t n, size_t m, size_t d, double const * v,
                         double * norm) {
    cal::AlignedVector <double> temp(n);

    cal::qa_list_dot(n, m, d, v, v, temp.data());

    cal::vsqrt(n, temp.data(), norm);
...
#pragma omp declare simd
void cal::qa_normalize_one(size_t d, double const * q_in,
                             double * q_out) {
    double norm = 0.0;
    for (size_t i = 0; i < d; ++i) {
        norm += q_in[i] * q_in[i];
...
void cal::qa_normalize(size_t n, size_t m, size_t d,
                         double const * q_in, double * q_out) {
    cal::AlignedVector <double> norm(n);

    cal::qa_amplitude(n, m, d, q_in, norm.data());

    if (cal::is_aligned(q_in) && cal::is_aligned(q_out)) {
        #pragma omp simd
        for (size_t i = 0; i < n; ++i) {
            size_t off = m * i;
            for (size_t j = 0; j < d; ++j) {
...
#pragma omp declare simd
void cal::qa_normalize_inplace_one(size_t d, double * q) {
    double norm = 0.0;
    for (size_t i = 0; i < d; ++i) {
        norm += q[i] * q[i];
    }
...
void cal::qa_normalize_inplace(size_t n, size_t m, size_t d, double * q) {
    cal::AlignedVector <double> norm(n);

    cal::qa_amplitude(n, m, d, q, norm.data());
...
#pragma omp declare simd
void cal::qa_rotate_one_one(double const * q, double const * v_in,
                              double * v_out) {
    double norm = 0.0;
...
void cal::qa_rotate_many_one(size_t nq, double const * q,
                               double const * v_in, double * v_out) {
    cal::AlignedVector <double> q_unit(4 * nq);

    cal::qa_normalize(nq, 4, 4, q, q_unit.data());

    if (cal::is_aligned(v_in) && cal::is_aligned(v_out)) {
        #pragma omp simd
...
void cal::qa_rotate_one_many(double const * q, size_t nv,
                               double const * v_in, double * v_out) {
    double norm = 0.0;
    for (size_t i = 0; i < 4; ++i) {
        norm += q[i] * q[i];
...
void cal::qa_rotate_many_many(size_t n, double const * q,
                                double const * v_in, double * v_out) {
    cal::AlignedVector <double> q_unit(4 * n);

    cal::qa_normalize(n, 4, 4, q, q_unit.data());
...
void cal::qa_rotate(size_t nq, double const * q, size_t nv,
                      double const * v_in, double * v_out) {
    if ((nq == 1) && (nv == 1)) {
        cal::qa_rotate_one_one(q, v_in, v_out);
    } else if (nq == 1) {
        cal::qa_rotate_one_many(q, nv, v_in, v_out);
    } else if (nv == 1) {
        cal::qa_rotate_many_one(nq, q, v_in, v_out);
    } else if (nq == nv) {
        cal::qa_rotate_many_many(nq, q, v_in, v_out);
    } else {
        auto here = cal_HERE();
...
#pragma omp declare simd
void cal::qa_mult_one_one(double const * p, double const * q,
                            double * r) {
...
void cal::qa_mult_one_many(double const * p, size_t nq,
                             double const * q, double * r) {
    if (cal::is_aligned(p) && cal::is_aligned(q) && cal::is_aligned(r)) {
        #pragma omp simd
        for (size_t i = 0; i < nq; ++i) {
            size_t f = 4 * i;
...
void cal::qa_mult_many_one(size_t np, double const * p,
                             double const * q, double * r) {
    if (cal::is_aligned(p) && cal::is_aligned(q) && cal::is_aligned(r)) {
        #pragma omp simd
...
void cal::qa_mult_many_many(size_t np, double const * p, size_t nq,
                              double const * q, double * r) {
    if (cal::is_aligned(p) && cal::is_aligned(q) && cal::is_aligned(r)) {
        #pragma omp simd
        for (size_t i = 0; i < np; ++i) {
...
void cal::qa_mult(size_t np, double const * p, size_t nq,
                    double const * q, double * r) {
    if ((np == 1) && (nq == 1)) {
        cal::qa_mult_one_one(p, q, r);
    } else if (np == 1) {
        cal::qa_mult_one_many(p, nq, q, r);
    } else if (nq == 1) {
        cal::qa_mult_many_one(np, p, q, r);
    } else if (np == nq) {
        cal::qa_mult_many_many(np, p, nq, q, r);
    } else {
        auto here = cal_HERE();
        auto log = cal::Logger::get();
...
void cal::qa_slerp(size_t n_time, size_t n_targettime,
...
void cal::qa_exp(size_t n, double const * q_in, double * q_out) {
    cal::AlignedVector <double> normv(n);

    cal::qa_amplitude(n, 4, 3, q_in, normv.data());
...
void cal::qa_ln(size_t n, double const * q_in, double * q_out) {
    cal::AlignedVector <double> normq(n);

    cal::qa_amplitude(n, 4, 4, q_in, normq.data());

    cal::qa_normalize(n, 4, 3, q_in, q_out);
...
void cal::qa_pow(size_t nq, size_t np, double const * p, double const * q_in,
                   double * q_out) {
    cal::AlignedVector <double> q_tmp(4 * nq);

    cal::qa_ln(nq, q_in, q_tmp.data());
    ...
    cal::qa_exp(nq, q_tmp.data(), q_out);
...
void cal::qa_from_axisangle_one_one(double const * axis,
...
void cal::qa_from_axisangle_one_many(size_t nang, double const * axis,
                                       double const * angle, double * q_out) {
    cal::AlignedVector <double> a(nang);
    if (cal::is_aligned(angle)) {
        #pragma omp simd
    ...
    cal::AlignedVector <double> sin_a(nang);
    cal::AlignedVector <double> cos_a(nang);

    cal::vsincos(nang, a.data(), sin_a.data(), cos_a.data());

    if (cal::is_aligned(axis) && cal::is_aligned(q_out)) {
        #pragma omp simd
        for (size_t i = 0; i < nang; ++i) {
            size_t off = 4 * i;
...
void cal::qa_from_axisangle_many_one(size_t naxis, double const * axis,
                                       double angle, double * q_out) {
...
void cal::qa_from_axisangle_many_many(size_t n, double const * axis,
                                        double const * angle, double * q_out) {
    cal::AlignedVector <double> a(n);
    ...
    cal::AlignedVector <double> sin_a(n);
    cal::AlignedVector <double> cos_a(n);

    cal::vsincos(n, a.data(), sin_a.data(), cos_a.data());
...
void cal::qa_from_axisangle(size_t naxis, double const * axis, size_t nang,
                              double const * angle, double * q_out) {
    if ((naxis == 1) && (nang == 1)) {
        cal::qa_from_axisangle_one_one(axis, angle[0], q_out);
    } else if (naxis == 1) {
        cal::qa_from_axisangle_one_many(nang, axis, angle, q_out);
    } else if (nang == 1) {
        cal::qa_from_axisangle_many_one(naxis, axis, angle[0], q_out);
    } else if (naxis == nang) {
        cal::qa_from_axisangle_many_many(naxis, axis, angle, q_out);
    } else {
        auto here = cal_HERE();
        auto log = cal::Logger::get();
        std::string msg(
...
void cal::qa_to_axisangle_one(double const * q, double * axis,
                                double * angle) {
...
void cal::qa_to_axisangle(size_t n, double const * q, double * axis,
                            double * angle) {
    if (cal::is_aligned(q) && cal::is_aligned(axis) &&
        cal::is_aligned(angle)) {
        #pragma omp simd
        for (size_t i = 0; i < n; ++i) {
            size_t qf = 4 * i;
            size_t vf = 3 * i;
...
void cal::qa_to_rotmat(double const * q, double * rotmat) {
    double xx = q[0] * q[0];
    double xy = q[0] * q[1];
    double xz = q[0] * q[2];
    double xw = q[0] * q[3];
...
void cal::qa_from_rotmat(const double * rotmat, double * q) {
...
void cal::qa_from_vectors(size_t n, double const * vec1,
                            double const * vec2, double * q) {
    if (cal::is_aligned(vec1) && cal::is_aligned(vec2) &&
        cal::is_aligned(q)) {
        #pragma omp simd
        for (size_t i = 0; i < n; ++i) {
            size_t vf = 3 * i;
            ...
            if (::fabs(dotprod - 1.0) < 1.0e-12) {
                ...
                cal::qa_normalize_inplace_one(4, &(q[qf]));
            }
        }
    } else {
        for (size_t i = 0; i < n; ++i) {
...
void cal::qa_from_angles(size_t n, double const * theta,
                           double const * phi, double * const pa,
                           double * quat, bool IAU) {
    if (cal::is_aligned(theta) && cal::is_aligned(phi) &&
        cal::is_aligned(pa) && cal::is_aligned(quat)) {
        #pragma omp simd
        for (size_t i = 0; i < n; ++i) {
            size_t qf = 4 * i;
            double qR[4];
            ...
            double angR = 0.5 * (phi[i] + cal::PI_2);
            ...
            cal::qa_mult_one_one(qD, qP, qtemp);
            cal::qa_mult_one_one(qR, qtemp, &(quat[qf]));
            cal::qa_normalize_inplace_one(4, &(quat[qf]));
        }
    } else {
        for (size_t i = 0; i < n; ++i) {
            size_t qf = 4 * i;
...
void cal::qa_to_angles(size_t n, double const * quat, double * theta,
    ...
    if (cal::is_aligned(theta) && cal::is_aligned(phi) &&
        ...
        for (size_t i = 0; i < n; ++i) {
            ...
            cal::qa_normalize_one(4, &(quat[qf]), qtemp);
            cal::qa_rotate_one_one(qtemp, zaxis, dir);
            cal::qa_rotate_one_one(qtemp, xaxis, orient);

            theta[i] = cal::PI_2 - ::asin(dir[2]);
...
void cal::qa_from_position(size_t n, double const * theta,
    ...
    if (cal::is_aligned(theta) && cal::is_aligned(phi)
        ...
        for (size_t i = 0; i < n; ++i) {
            ...
            cal::qa_mult_one_one(qR, qD, &(quat[qf]));
            cal::qa_normalize_inplace_one(4, &(quat[qf]));
        }
    } else {
        for (size_t i = 0; i < n; ++i) {
...
void cal::qa_to_position(size_t n, double const * quat, double * theta,
                           double * phi) {
    if (cal::is_aligned(theta) && cal::is_aligned(phi) &&
        cal::is_aligned(quat)) {
        #pragma omp simd
