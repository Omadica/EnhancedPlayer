...
void cal::vcos(int n, double const * ang, double * cosout) {
    vmdCos(n, ang, cosout, VML_HA | VML_FTZDAZ_OFF | VML_ERRMODE_DEFAULT);
    return;
}

void cal::vsincos(int n, double const * ang, double * sinout,
                    double * cosout) {
...
void cal::vatan2(int n, double const * y, double const * x,
                   double * ang) {
    vmdAtan2(n, y, x, ang, VML_HA | VML_FTZDAZ_OFF | VML_ERRMODE_DEFAULT);
    return;
}

void cal::vsqrt(int n, double const * in, double * out) {
    vmdSqrt(n, in, out, VML_HA | VML_FTZDAZ_OFF | VML_ERRMODE_DEFAULT);
    return;
}

void cal::vrsqrt(int n, double const * in, double * out) {
    vmdInvSqrt(n, in, out, VML_HA | VML_FTZDAZ_OFF | VML_ERRMODE_DEFAULT);
    return;
}

void cal::vexp(int n, double const * in, double * out) {
    vmdExp(n, in, out, VML_HA | VML_FTZDAZ_OFF | VML_ERRMODE_DEFAULT);
    return;
}

void cal::vlog(int n, double const * in, double * out) {
...
void cal::vfast_cos(int n, double const * ang, double * cosout) {
    vmdCos(n, ang, cosout, VML_LA | VML_FTZDAZ_OFF | VML_ERRMODE_DEFAULT);
    return;
}

void cal::vfast_sincos(int n, double const * ang, double * sinout,
                         double * cosout) {
    vmdSinCos(n, ang, sinout, cosout,
...
void cal::vfast_atan2(int n, double const * y, double const * x,
                        double * ang) {
    vmdAtan2(n, y, x, ang, VML_LA | VML_FTZDAZ_OFF | VML_ERRMODE_DEFAULT);
    return;
}

void cal::vfast_sqrt(int n, double const * in, double * out) {
    vmdSqrt(n, in, out, VML_LA | VML_FTZDAZ_OFF | VML_ERRMODE_DEFAULT);
    return;
}

void cal::vfast_rsqrt(int n, double const * in, double * out) {
    vmdInvSqrt(n, in, out, VML_LA | VML_FTZDAZ_OFF | VML_ERRMODE_DEFAULT);
    return;
}

void cal::vfast_exp(int n, double const * in, double * out) {
    vmdExp(n, in, out, VML_LA | VML_FTZDAZ_OFF | VML_ERRMODE_DEFAULT);
    return;
}

void cal::vfast_log(int n, double const * in, double * out) {
    vmdLn(n, in, out, VML_LA | VML_FTZDAZ_OFF | VML_ERRMODE_DEFAULT);
    return;
}

void cal::vfast_erfinv(int n, double const * in, double * out) {
...
void cal::vcos(int n, double const * ang, double * cosout) {
    if (cal::is_aligned(ang) && cal::is_aligned(cosout)) {
        # pragma omp simd
        for (int i = 0; i < n; ++i) {
            cosout[i] = ::cos(ang[i]);
        }
...
void cal::vatan2(int n, double const * y, double const * x,
                   double * ang) {
    if (cal::is_aligned(ang) && cal::is_aligned(x) &&
        cal::is_aligned(y)) {
        # pragma omp simd
        for (int i = 0; i < n; ++i) {
            ang[i] = ::atan2(y[i], x[i]);
...
void cal::vsqrt(int n, double const * in, double * out) {
    if (cal::is_aligned(in) && cal::is_aligned(out)) {
        # pragma omp simd
        for (int i = 0; i < n; ++i) {
            out[i] = ::sqrt(in[i]);
        }
...
void cal::vfast_sin(int n, double const * ang, double * sinout) {
    ...
    for (i = 0; i < n; i++) {
        ...
        while (rang > cal::TWOPI) {
            rang -= cal::TWOPI;
        }
        if ((fabs(rang) < eps) || (fabs(rang - cal::TWOPI) < eps)) {
            sinout[i] = 0.0;
            continue;
        }
        quot = ang[i] * cal::INV_TWOPI;
        rem = quot - floor(quot);
        x = rem * cal::TWOPI;
        while (x < 0.0) {
            x += cal::TWOPI;
        }
        quad = static_cast <int> (x * cal::TWOINVPI);
...
void cal::vfast_atan2(int n, double const * y, double const * x,
    ...
    for (i = 0; i < n; i++) {
        ...
        if (r > cal::TANTWELFTHPI) {
            r = (r - cal::TANSIXTHPI) / (1 + cal::TANSIXTHPI * r);
            region = 1;
        }
        r2 = r * r;
...
void cal::vfast_erfinv(int n, double const * in, double * out) {
    ...
    //
    cal::AlignedVector <double> arg(n);
    cal::AlignedVector <double> lg(n);

    if (cal::is_aligned(in)) {
        # pragma omp simd
        for (int i = 0; i < n; ++i) {
            double ab = ::fabs(in[i]);
            arg[i] = (1.0 - ab) * (1.0 + ab);
    ...
    cal::vfast_log(n, arg.data(), lg.data());

    if (cal::is_aligned(out)) {
        # pragma omp simd
        for (int i = 0; i < n; ++i) {
            double w = -lg[i];
            double p;
